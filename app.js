...
app.get('/library/getCategory', function (req, res) {
    db.query('SELECT * FROM library_category', function (error, result) {
        if (error) {
            console.log(error);
        }
        res.send(result);
...
app.get('/library/getBookList/:cid', function (req, res) {
    db.query(`SELECT * FROM library_book WHERE cid = ${req.params.cid}`, function (error, result) {
        if (error) {
            console.log(error);
        }
        res.send(result);
...
app.get('/library/getBookDetail/:id', function (req, res) {
    db.query(`SELECT a.*, b.name as categoryName FROM library_book a JOIN library_category b ON a.cid = b.id WHERE a.id = ${req.params.id}`, function (error, result) {
        if (error) {
            console.log(error);
        }
        res.send(result);
